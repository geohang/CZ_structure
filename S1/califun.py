...
def califun(K_layer1,K_layer2,K_layer3,porosity1,porosity2,porosity3,rate,ani,ani2):
    ...
    initwc[0] = np.ones((k33.shape[0],k33.shape[1]))*(porosity1*0.5)
    initwc[1] = np.ones((k33.shape[0],k33.shape[1]))*(porosity2*0.5)
    ...
    uz_ts1 = np.loadtxt(os.path.join(dat_pth, "TLoutflow_11_15_CC1.txt"))
    ...
    uz_et1 = np.loadtxt(os.path.join(dat_pth, "TLET_11_15_CC1.txt"))
    ...
    os.mkdir('/bsuhome/hangchen/scratch/Test9/'+'Tdatafile' + str(fileID))
    sim_ws = '/bsuhome/hangchen/scratch/Test9/'+'Tdatafile' + str(fileID)
...
#       temp[temp>=0]=0     
       ...
       if len(outflow_BB) < num_ts:
           ...
           ## define the start
           T_start = 365*10
           res_path = '/bsuhome/hangchen/scratch/Res9/'+'S5_res' + str(fileID)
           os.mkdir(res_path)
